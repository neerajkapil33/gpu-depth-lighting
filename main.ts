...
import { detectFace, setupFaceTracking, type FacePose } from './face-tracking.ts';
...
let frames=0;
const settings={...defaultRelightingSettings,lightPosition:[...defaultRelightingSettings.lightPosition] as [number,number],lightColor:[...defaultRelightingSettings.lightColor] as [number,number,number],facePosition:[0.5,0.42] as [number,number],faceSize:[0.22,0.28] as [number,number],faceActive:0,handPosition:[0.5,0.5] as [number,number],handSize:[0.18,0.22] as [number,number],handActive:0};
function setStatus(text:string){status.textContent=text;}
function syncRenderer(){renderer?.update({lightPosition:settings.lightPosition,lightZ:settings.lightZ,intensity:lightActive?settings.intensity:0,exposure:settings.exposure,relief:settings.relief,shadow:settings.shadow,occlusion:settings.occlusion,lightColor:settings.lightColor,mirror:settings.mirror,mode:settings.mode,facePosition:settings.facePosition,faceSize:settings.faceSize,faceActive:settings.faceActive,handPosition:settings.handPosition,handSize:settings.handSize,handActive:settings.handActive});}
...
function bindControls(){
 ...
 document.getElementById('ctrl-reset')?.addEventListener('click',()=>{Object.assign(settings,{...defaultRelightingSettings,lightPosition:[...defaultRelightingSettings.lightPosition] as [number,number],lightColor:[...defaultRelightingSettings.lightColor] as [number,number,number],facePosition:[0.5,0.42] as [number,number],faceSize:[0.22,0.28] as [number,number],faceActive:0,handPosition:[0.5,0.5] as [number,number],handSize:[0.18,0.22] as [number,number],handActive:0});setLightActive(true);syncRenderer();});
...
function updateFace(now:number){face=detectFace(video,now);if(!face){settings.faceActive=0;return;}settings.facePosition=[face.x,face.y];settings.faceSize=[Math.max(face.width,0.16),Math.max(face.height,0.20)];settings.faceActive=1;}
function updatePalm(now:number){if(!palmControl||!hand||!lightActive){handVisible=false;settings.handActive=0;return;}const result:HandLandmarkerResult=hand.detectForVideo(video,now);const lm=result.landmarks[0];if(!lm){handVisible=false;settings.handActive=0;return;}let x=0,y=0,minX=1,minY=1,maxX=0,maxY=0;for(const i of [0,5,9,13,17]){x+=lm[i].x;y+=lm[i].y;}for(const p of lm){minX=Math.min(minX,p.x);minY=Math.min(minY,p.y);maxX=Math.max(maxX,p.x);maxY=Math.max(maxY,p.y);}x/=5;y/=5;settings.lightPosition=[x,y];settings.handPosition=[(minX+maxX)*0.5,(minY+maxY)*0.5];settings.handSize=[Math.max((maxX-minX)*0.55,0.12),Math.max((maxY-minY)*0.55,0.16)];settings.handActive=1;renderer?.update({lightPosition:settings.lightPosition,handPosition:settings.handPosition,handSize:settings.handSize,handActive:1});handVisible=true;if(handIndicator){handIndicator.style.left=`${x*canvas.clientWidth}px`;handIndicator.style.top=`${y*canvas.clientHeight}px`;}}
...
function renderFrame(now:number){if(!running||!renderer)return;const t0=performance.now();updateFace(now);updatePalm(now);if(lightActive&&!palmControl)lightInput.orbitTick();syncRenderer();renderer.render({source:video,uvTransform:d.mat2x2f.identity(),swapAxes:false});updateBulb();frames++;if(now-last>500){fpsEl&&(fpsEl.textContent=`${Math.round(frames*1000/(now-last))} fps`);frames=0;last=now;}cpuMs&&(cpuMs.textContent=`${(performance.now()-t0).toFixed(1)} ms`);}
