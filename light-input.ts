...
export function setupLightInput(canvas: HTMLCanvasElement, onChange: (update: LightUpdate) => void, signal: AbortSignal): LightInput {
  ...
  const fraction = (e: PointerEvent) => { const r = canvas.getBoundingClientRect(); if (!r.width || !r.height) return undefined; return { x: (e.clientX-r.left)/r.width, y: (e.clientY-r.top)/r.height }; };
  ...
  function move(e: PointerEvent) {
    ...
    if(gesture.kind==='pinch'){const s=span(); if(gesture.span>0)pushLight((s-gesture.span)*PINCH_SENSITIVITY); gesture.span=s; return;}
    const p=fraction(e); if(!p)return;
